import { Button, Card, CardBody, CardFooter, CardHeader, Chip, Divider, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Popover, PopoverContent, PopoverTrigger, Switch, Tooltip } from '@nextui-org/react';
...
import { COLORS, command_t, key_config_t, key_modifier_t, node_info_t, node_mode_t, node_state_default_t, peer_data_t } from "./util";

import * as material from 'material-colors';

type PeerInfoProps = {
    sendCommand: (peer: peer_data_t, command: command_t, args?: number[]) => void,
    peer: peer_data_t,
...
export function PeerInfo(props: PeerInfoProps) {
    ...
    const buzz = useCallback(() => sendCommand(peer, command_t.COMMAND_BUZZ), [sendCommand, peer]);
    ...
    const setActive = useCallback((active: boolean) => { sendCommand(peer, active ? command_t.COMMAND_SET_ACTIVE : command_t.COMMAND_SET_INACTIVE); _setActive(active); }, [sendCommand, peer]);
    ...
    }), [peer, sendCommand, setShowKeybindPicker, setShowColorPicker]);
    ...
    return <>
        <Card className={classNames("transition",
            ...
            <Divider />
            <div className="flex flex-row flex-nowrap gap-0 p-0 justify-stretch py-0">
                <Button variant={peer.node_info.current_mode == node_mode_t.MODE_DEFAULT ? "flat" : "light"} color='default' radius='none' className="px-5 flex-1" isDisabled={!peer.valid_version} onPress={() => sendCommand(peer, command_t.COMMAND_SET_MODE, [node_mode_t.MODE_DEFAULT])}>Standard</Button>
                <Button variant={peer.node_info.current_mode == node_mode_t.MODE_SIMON_SAYS ? "flat" : "light"} color='default' radius='none' className="px-5 flex-1" isDisabled={!peer.valid_version} onPress={() => sendCommand(peer, command_t.COMMAND_SET_MODE, [node_mode_t.MODE_SIMON_SAYS])}>Simon Says</Button>
            </div>
            <Divider />

            <CardBody>
                <Button
                    isDisabled={!peer.valid_version || peer.node_info.current_mode != node_mode_t.MODE_DEFAULT}
            ...
            <Popover className="dark text-foreground " placement="right" backdrop='transparent' isOpen={showColorPicker} onOpenChange={(open) => setShowColorPicker(open)}>
                ...
                <PopoverContent className="p-5">
                    {/* <CirclePicker color={color} colors={PICKER_COLORS} onChange={v => sendCommand(peer, command_t.COMMAND_SET_COLOR, [0xFF, v.rgb.r, v.rgb.g, v.rgb.b])} /> */}
                    <SketchPicker color={color} presetColors={PICKER_COLORS} onChangeComplete={v => sendCommand(peer, command_t.COMMAND_SET_COLOR, [0xFF, v.rgb.r, v.rgb.g, v.rgb.b])} />
                </PopoverContent>
            </Popover>
