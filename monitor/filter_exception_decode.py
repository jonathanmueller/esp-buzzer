...
class CustomEsp32ExceptionDecoder(DeviceMonitorFilterBase):
    NAME = "custom_esp32_exception_decoder"

    ADDR_PATTERN = re.compile(r"((?:0x[0-9a-fA-F]{8}[: ]?)+)\s?\b")
    ADDR_SPLIT = re.compile(r"[ :]")
    PREFIX_RE = re.compile(r"^ *")
    STACK_MEMORY_HEADER = r"Stack memory:"

    def __call__(self):
        self.buffer = ""
        self.in_stack_memory_dump = False
...
"""
    ...
    def rx(self, text):
        ...
        while True:
            ...
            if line.startswith(self.STACK_MEMORY_HEADER):
                self.in_stack_memory_dump = True
                self.stack_addresses = []
                continue

            if self.in_stack_memory_dump:
                if line.strip() == "":
                    trace = "Decoded Stack:\n" + self.insert_backtrace_from_addresses(self.stack_addresses)
                    ...
                    self.stack_addresses += self.filter_addresses(line)
                    continue
            else:
    ...
    def filter_addresses(self, adresses_str):
        addresses = []
        for addr in self.ADDR_PATTERN.findall(adresses_str):
            addresses += self.ADDR_SPLIT.split(addr)
        
        return [a for a in addresses if not self.is_address_ignored(addr)]
    ...
    def build_backtrace(self, line, address_match):
        ...
        return self.insert_backtrace_from_addresses(addresses, prefix)

    def insert_backtrace_from_addresses(self,addresses, prefix = ""):
        trace = ""
        ...
        try:
            ...
            for addr in addresses:
                output = (
                    subprocess.check_output(args + [addr], timeout=1000)
                    .decode(enc)
                    .strip()
                )
